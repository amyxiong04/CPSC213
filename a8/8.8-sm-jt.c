...
void fetch() {
  ...
  insOpImm  = mem [pc+1];
  pc += 2;
  static const void* jt[] = { &&L0x0, &&L0xb, &&DEFAULT};
  if (insOpCode == 0x0) {
    goto *jt [0];
  } else if (insOpCode == 0xb) {
      goto *jt [1];
  } else { 
      goto *jt [2];
  }
...
int cont = 1;
int addr, val;
void* jt1[16] = {&&L0,&&L1,&&L2,&&L3,&&L4,0,&&L6,&&L7,&&L8,&&L9,&&La,&&Lb,&&Lc,0,0,&&Lf};
void* jt2[16] = {&&A0,&&A1,&&A2,&&A3,&&A4,&&A5,&&A6,&&A7,0,0,0,0,0,0,0,&&Af};
if (insOpCode < 0x0|| insOpCode > 0xf|| insOpCode == 0x5 || insOpCode == 0xd || insOpCode ==0xe || insOpCode > 0xf) goto DEFAULT;
goto *jt1[insOpCode];
L0:
  reg [insOp0] = insOpExt;
  goto CONT;
...
L4:
  ...
  mem [addr]= val >> 24 & 0xff;
  mem [addr+1] = val >> 16 & 0xff;
  mem [addr+2] = val >>8 & 0xff;mem [addr+3] = val& 0xff;
  goto CONT;
L6:
